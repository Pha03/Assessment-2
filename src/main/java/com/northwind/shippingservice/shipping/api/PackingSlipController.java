...
public class PackingSlipController {
    ...
    public ResponseEntity<List<PackingSlipModel>> get(@RequestParam(required = false) Optional<Integer> offset, @RequestParam(required = false) Optional<Integer> limit) {
        ...
        int take = limit.orElse(50); //default limit to 50 if not specified
        if (take > 50) {
            throw new IllegalArgumentException("Limit cannot be more than 50.");
        }
        List<PackingSlipModel> customers = service.getAll(skip, take)
                .stream().map(c -> PackingSlipMapper.toModel(c))
    ...
    public ResponseEntity<PackingSlipModel> create(@RequestBody PackingSlipModel model) {
        ...
        return ResponseEntity.created(URI.create("/PackingSlips/" + savedShipping.getId()))
    ...
    public ResponseEntity<List<PackingSlipModel>> findByCustomerName(@RequestParam String shipName) {
